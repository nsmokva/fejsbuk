						...
						<v-col v-for="user in users" :key="user.name" cols="6" class="pb-0">
							<v-card flat outlined min-width="350">
								<v-row>
									...
									<v-col cols="auto" class="primary--text font-weight-medium text-body-2 n-pointer" align-self="center">
					...
					<v-row class="ma-0 pa-1">
						<v-col v-for="user in nineUsers" :key="user.name" cols="4" class="pa-1">
							<v-card tile elevation="0" :to= "{ name: 'timeline', params: { id: user._id }}">
								<v-img v-if="user.photoName != undefined && user.photoName != ''" :src="'/backend/user/images/' + user.photoName" width="auto" aspect-ratio='1'></v-img>
								<v-img v-else :src="require('../../public/default.jpeg')" width="auto" aspect-ratio='1'></v-img>
								<span class="primary--text font-weight-medium text-body-2 n-pointer">{{user.firstName}} {{user.lastName}}</span>
							</v-card>
						</v-col>
					</v-row>
				</v-card>
